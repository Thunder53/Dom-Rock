...
async function cadastrar() {
  ...
  try {
    const clienteResponse = await fetch(
      `http://localhost:8080/cliente/id-por-nome?nome_cliente=${clienteNome}`
    );
    const cliente = await clienteResponse.json();

    const produtoResponse = await fetch(
      `http://localhost:8080/produto/id-por-nome?nome_produto=${produtoNome}`
    );
...
fetch(
  `http://localhost:8080/clientes-by-vendedor/${localStorage.getItem("id")}`
)
