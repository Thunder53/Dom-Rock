fetch('http://localhost:8080/venda')
  ...
  .then(data => {
    ...
    // Adiciona um evento de clique ao botão "Filtrar por data"
botaoFiltrarData.addEventListener('click', function() {
    // Pega o valor do input do usuário
    const dataFiltrada = inputData.value;
    
    // Filtra os dados com base no valor do input
    const dadosFiltrados = dados.filter(dado => {
      if (dataFiltrada && dado.data !== dataFiltrada) {
        return false;
      }
      return true;
    });
  ...
  // Adiciona um evento de clique ao botão "Filtrar por produto"
  botaoFiltrarProduto.addEventListener('click', function() {
    // Pega o valor do input do usuário
    const produtoFiltrado = inputProduto.value;
    
    // Filtra os dados com base no valor do input
    const dadosFiltrados = dados.filter(dado => {
      if (produtoFiltrado && dado.produto !== produtoFiltrado) {
        return false;
      }
      return true;
    });
  ...
  function atualizarGraficos(dados) {
    ...
    grafico1.atualizarDados(dados);
  
    // Atualiza o segundo gráfico
    grafico2.atualizarDados(dados);
  }
